...
data "oci_core_subnets" "sn_db_nprd_subnet" {
    compartment_id     = local.network_compartment_ocid
	filter {
        name            = "display_name"
        values           = ["${var.sn_db_nprd_display_name}"]
    }
}
...
data "oci_core_subnets" "sn_sec_db_nprd_subnet" {
    compartment_id     = local.network_compartment_ocid
	filter {
        name            = "display_name"
        values           = ["${var.sn_sec_db_nprd_display_name}"]
    }
}
