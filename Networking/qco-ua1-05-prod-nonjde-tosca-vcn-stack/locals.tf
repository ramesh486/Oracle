locals {
  ...
  vcn_nonjde_routetable_private_route_rules = flatten([
    local.common_route_rules_for_all_subnets
    # Add specific route rules if required
  ])

  vcn_nonjde_destination_security_rules = [
    {
      ...
      "security_list_egress_security_rules_stateless"        = var.tosca_vcn_nonjde_securitylist_is_stateless
    }
  ]

  vcn_nonjde_source_security_rules = [
    {
      "security_list_ingress_security_rules_source"      = var.tosca_vcn_nonjde_cidr_block
      ...
      "security_list_ingress_security_rules_stateless"   = var.tosca_vcn_nonjde_securitylist_is_stateless
    }
  ]
